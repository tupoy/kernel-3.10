...
static int stk8312_local_init(void);
static int stk8312_remove(void);
static int stk8312_init_flag =-1; // 0<==>OK -1 <==> fail

/*---------------------------------------------------------------------------*/
static struct acc_hw stk8312_cust_acc_hw = {
    .i2c_num = 1,
    .direction = 6,//5,
...
/*---------------------------------------------------------------------------*/
struct acc_hw* stk8312_get_cust_acc_hw(void) 
{
    return &stk8312_cust_acc_hw;
}
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info stk8312_init_info = {
		.name = "stk8312",
		.init = stk8312_local_init,
		.uninit = stk8312_remove,
};
	...
	if(atomic_read(&obj->filter))
	{
		if(atomic_read(&obj->fir_en) && !atomic_read(&obj->suspend))
		{
			...
			else
			{
				...
				obj->fir.idx++;
//#ifdef STK_ZG_FILTER	
//				if( abs(obj->fir.sum[STK831X_AXIS_X]/firlen) <= STK_ZG_COUNT)
//				{
//					data[STK831X_AXIS_X]  = (obj->fir.sum[STK831X_AXIS_X]*zero_fir)/firlen;
//				}
//				else
//				{
					data[STK831X_AXIS_X]  = obj->fir.sum[STK831X_AXIS_X]/firlen;
//				}	
//				if( abs(obj->fir.sum[STK831X_AXIS_Y]/firlen) <= STK_ZG_COUNT)
//				{
//					data[STK831X_AXIS_Y]  = (obj->fir.sum[STK831X_AXIS_Y]*zero_fir)/firlen;
//				}
//				else
//				{
					data[STK831X_AXIS_Y]  = obj->fir.sum[STK831X_AXIS_Y]/firlen;
//				}	
//				if( abs(obj->fir.sum[STK831X_AXIS_Z]/firlen) <= STK_ZG_COUNT)
//				{
//					data[STK831X_AXIS_Z]  = (obj->fir.sum[STK831X_AXIS_Z]*zero_fir)/firlen;
//				}
//				else
//				{
...
/*----------------------------------------------------------------------------*/
static int stk8312_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int stk831x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = stk8312_get_cust_acc_hw();
	...
	if((err = stk831x_create_attr(&(stk8312_init_info.platform_diver_addr->driver))))
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = stk8312_operate;
...
static int stk831x_i2c_remove(struct i2c_client *client)
{
	...
	if((err = stk831x_delete_attr(&(stk8312_init_info.platform_diver_addr->driver))))
	{
...
{
	struct acc_hw *hw = stk8312_get_cust_acc_hw();
	GSE_FUN();
...
#endif
    hwmsen_gsensor_add(&stk8312_init_info);
#if 0
